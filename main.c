...
void fill_rect(int x, int y, int w, int h, int color, unsigned int lineLength) {
	for(int i = 0; i < h; i++) {
		unsigned int py = y + i;
		// X coords and width multiplied by 2 because bpp is 16
		memset(fbp + (py * lineLength + x * 2 + curPage * pageSize), color, w * 2);
	}
}
...
void render_screen() {
	...
	int pw = vinfo.xres/screen_width;
	int ph = vinfo.yres/screen_height;
	for(i = 0; i < pixel_number; i++) {
		if(canvas_data[0][i]) {
			int x = (i % screen_width) * pw;
			int y = floor(i / screen_width) * ph;
			fill_rect(x, y, pw, ph, 255, lineLength);
		}
	}
}
...
void keyboardMonitor() {
	...
	if(pid2 == 0) {
		...
		if((infd = open("/dev/input/by-id/usb-_Raspberry_Pi_Internal_Keyboard-event-kbd", O_RDONLY)) < 0) {
